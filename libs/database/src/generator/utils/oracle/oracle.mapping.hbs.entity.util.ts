...
export function oracleEntityHbsMapping(
  ...
  for (const column of columns) {
    const current: OracleEntityHbsData['columns'][number] = {
      ...
      dataType: oracleDataTypeMapping(column),
  ...
  if (tableConstraints.length) {
    ...
    if (uniqueRows.length) {
      ...
      for (const row of uniqueRows) {
        ...
        uniqueMap.get(name)!.push(colName);
      }
      ...
      response.constraints.foreign = Array.from(fkMap.entries()).map(
        ([constraintName, rows]) => {
          ...
          const columnsFk = rows.map(row => {
            ...
            const referencedColumnName = localColName;
            const referencedColumnNameTS = toCamelCase(referencedColumnName);
  ...
  if (indexes.length) {
    ...
    if (tableIndexes.length) {
      const indexMap = new Map<
        string,
        { indexName: string; columnNames: string[]; isUnique: boolean }
      >();
      tableIndexes.sort(
        (a, b) =>
