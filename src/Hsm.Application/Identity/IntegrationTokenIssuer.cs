    ...
    IEnvironmentPolicy environment)
{
    ...
    /// SHA-256 pre-digest before hashing existed only to work around bcrypt's
    /// 72-byte truncation — with bcrypt gone, so is the reason for the
    ...
    /// <see cref="IUnitOfWork.ExecuteInTransactionAsync"/>, which joins the
    ...
    public async Task<IntegrationTokens> IssueAsync(AuthPrincipal principal, CancellationToken ct = default)
    {
        ...
        // The developer environment gate.
