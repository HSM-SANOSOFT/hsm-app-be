    ...
    : IRequestHandler<ForgotPasswordCommand, Unit>
{
    public async Task<Unit> HandleAsync(ForgotPasswordCommand request, CancellationToken ct)
    {
        ...
        {
            // An unusable link must not burn a rate-limit slot, so the attempt
            // is simply not recorded. Stay generic — surfacing a 500 only for
    ...
    /// <summary>
    /// The account's reset requests inside the ROLLING hour (at most five).
    /// The window used to be inferred from the created-at timestamps of the
    /// reset-token rows; with those rows gone it is kept explicitly, as the
    /// most recent request timestamps in an Identity user token — the same
    /// table (user_tokens) reset state would live in if Identity's own tokens
    /// were stateful.
    /// </summary>
