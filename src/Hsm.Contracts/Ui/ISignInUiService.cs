...
/// and issues THE SAME auth cookies the REST login sets — one session model
...
/// call to POST /v1/auth/login instead.
/// </summary>
public interface ISignInUiService
{
    /// <summary>
    /// Validates the credentials and, on success, establishes the browser
    /// session (auth cookies on the current HTTP response). Never throws for
    /// bad credentials — the failure rides in the result so the screen can
    /// render it.
    /// </summary>
