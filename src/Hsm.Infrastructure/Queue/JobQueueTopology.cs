...
/// <param name="InitialDelay">Delay before the FIRST attempt.</param>
...
/// (<c>Queue:KeyPrefix</c>) with a fixed default, never derived per host or per
...
/// slow one; it can only measure idle time. There is no mid-flight lock
/// renewal here, so a long-running job looks the same as a dead consumer's —
/// the only thing standing between a slow job and a duplicate run is
