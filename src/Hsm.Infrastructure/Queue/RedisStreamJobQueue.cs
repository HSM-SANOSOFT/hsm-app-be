...
/// instances each held its own invisible queue — a regression that no test
/// caught because every test ran one process. A stream entry survives the
/// process that wrote it, and a consumer group makes exactly one consumer —
/// in any process — take each entry.</para>
///
...
/// docs queue's 1s) goes onto the delayed set instead of the stream, so the
/// delay is stored rather than slept through by the enqueuing request.</para>
/// </summary>
