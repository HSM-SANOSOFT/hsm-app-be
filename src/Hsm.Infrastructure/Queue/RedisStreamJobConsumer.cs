...
///
/// <para>Delivery is at-least-once: a consumer that dies mid-job leaves its
/// entry pending, XAUTOCLAIM hands it to
    ...
    ILogger<RedisStreamJobConsumer> logger) : IJobConsumer
{
    ...
    public async Task<int> DrainOnceAsync(string queue, int max, CancellationToken ct)
    {
        ...
        // Due work — retries, and first attempts that waited out the
